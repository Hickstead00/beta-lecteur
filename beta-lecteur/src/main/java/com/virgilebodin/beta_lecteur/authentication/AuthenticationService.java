...
import com.virgilebodin.beta_lecteur.security.JwtService;
...
public class AuthenticationService {
    ...
    public void activateAccount(String token) throws MessagingException {
        Token savedToken = tokenRepository.findByToken(token)
                .orElseThrow(() -> new RuntimeException("Le token n'est pas valide"));
        if(LocalDateTime.now().isAfter(savedToken.getExpiresAt())) {
            sendValidationEmail(savedToken.getUser());
            throw new RuntimeException("Le token d'activation à expiré. Un nouveau token à été envoyé à la même adresse mail");
        }

        var user = userRepository.findById(savedToken.getUser().getId())
                .orElseThrow(() -> new UsernameNotFoundException("L'utilisateur n'est pas valide"));
